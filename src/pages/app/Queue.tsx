  ...
  return (
    <>
      <TopBar
        ...
        subtitle="Drafts awaiting review · nothing posts without you"
        actions={
        ...
        {items.length === 0 ? (
          ...
          <Card key={d.id} className="p-5">
            <div className="flex items-start justify-between gap-4 mb-3">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-muted-foreground">
                  Re: <span className="font-mono">@{d.tweet?.source_handle}</span> — “{d.tweet?.text?.slice(0, 100)}…”
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4 p-3 rounded-md bg-surface-2 border border-border">
              <ScoreBar label="Topic" value={d.classification?.topic_score ?? 0} />
              <ScoreBar label="Source" value={d.classification?.source_score ?? 0} />
              <ScoreBar label="Action" value={d.classification?.actionability_score ?? 0} />
              <ScoreBar label="Risk" value={d.classification?.risk_score ?? 0} variant="risk" />
            </div>
            ...
            {editing === d.id ? (
              <div className="space-y-2">
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <Button size="sm" onClick={() => saveEdit(d.id)}>Save</Button>
                </div>
              </div>
            ) : (
...
function StatusBadge({ status }: { status: string }) {
  ...
  return <Badge variant="outline" className={`text-[10px] font-mono uppercase tracking-wider ${map[status] ?? ""}`}>{status}</Badge>;
}
